...
const state=初始状态();
const persistedAllowed=Array.isArray(patch.allowedRemovedSkuIds)&&patch.allowedRemovedSkuIds.length?patch.allowedRemovedSkuIds:(patch.deletedIds||[]);
...
function 陈列图基础行(store){const seen=new Set();return 纳入SKU(store).filter(r=>{if(r.inStaging)return false;const duplicateKey=r.lifecycleTaskId&&!r.placementCloneOf&&!r.customPlacement?[r.lifecycleTaskId,r.lifecycleTaskRowId||r.id].join("||"):"";if(duplicateKey&&seen.has(duplicateKey))return false;if(duplicateKey)seen.add(duplicateKey);return true})}
...
window.空位方案缓存={};
function 柜段内SKU(store,cabKey){const seen=new Set();return 纳入SKU(store).filter(r=>{if(r.cabinetKey!==cabKey)return false;const k=r.lifecycleTaskId&&!r.placementCloneOf&&!r.customPlacement?[r.lifecycleTaskId,r.lifecycleTaskRowId||r.id].join("||"):"";if(k&&seen.has(k))return false;if(k)seen.add(k);return true})}
...
window.追加新增门店=()=>{const pre=window.新增门店测算缓存;if(!pre){alert("请先测算新增门店");return}if(状态.stores.some(s=>s.store===pre.store)){alert("门店已存在，不能重复追加");return}const v=pre.validation||严格复核新增门店(pre);if(!v.ok){alert("严格复核不通过，不能追加：\\n"+v.errors.join("\\n"));return}const normalize=window.PlanogramProjection?.normalizeNewStorePlanogramRows;if(!normalize){alert("新增门店模块转换尚未加载，请刷新页面后重试。");return}const newRows=normalize(pre.skus);刷新已加载陈列容量({params:状态.params,cabinets:pre.cabinets,skus:newRows});状态.stores.push({store:pre.store,type:pre.type,vertical:pre.cabinets.filter(c=>c.kind==="立柜").length?"自定义":"",chest:pre.cabinets.filter(c=>c.kind==="卧柜").length?"自定义":"",ice:pre.cabinets.filter(c=>c.kind.includes("冰淇淋")).length?"自定义":""});状态.cabinets.push(...pre.cabinets.map(c=>{const x={...c};delete x.items;delete x.used;delete x.left;return x}));状态.skus.push(...newRows);当前.门店=pre.store;保存();渲染全部();切换("displaymap");完成提示("新增门店已追加到当前页面，请在陈列图中继续手动调整。")}
...
function 可见SKU陈列模块(r){
 const key=产品主键(r)||SKU键(r);
 if(!key)return [];
 return 陈列图行投影(r?.store).filter(x=>(产品主键(x)||SKU键(x))===key);
}
function SKU实际模块数(r){return 可见SKU陈列模块(r).length}
function SKU首个实际模块(r){return 可见SKU陈列模块(r)[0]}
function 同SKU同柜段已有模块(r,targetKey){
 return 可见SKU陈列模块(r).some(x=>x.sourceRowId!==r?.id&&x.cabinetKey===targetKey);
}
function 陈列图基础校验(r,targetKey,allowSameCabinet=false){
  ...
  return {ok:true,source,target,layout,need:columns*layout.faceWidth}
}
...
function 陈列图池SKU(store){
  ...
  if(type==="unplaced")return rows.filter(r=>!SKU已纳入(r)&&!文(r.status).includes("淘汰"));
...
function 渲染陈列图右侧(){
  ...
  if(selected){
    ...
    const modules=同SKU陈列模块(selected),visibleModules=可见SKU陈列模块(selected),displayModules=visibleModules.concat(modules.filter(m=>m.inStaging));
    const selectedVisible=displayModules.find(m=>(m.sourceRowId||m.id)===selected.id);
    const location=selected.inStaging?'待选区':selectedVisible?柜名(selectedVisible)+' · '+柜位(selectedVisible):'未显示（陈列模块与当前柜体不匹配）';
    const moduleIndex=Math.max(0,displayModules.findIndex(x=>(x.sourceRowId||x.id)===selected.id))+1;
    const moduleRows=displayModules.map((m,i)=>{const sourceId=m.sourceRowId||m.id;return '<div class="module-row '+(sourceId===selected.id?'active':'')+'"><span>模块'+(i+1)+' · '+逃(冰柜类型(陈列图来源柜段(m)))+' · '+逃(m.inStaging?'待选区':柜名(m)+' '+柜位(m))+'</span>'+(可编辑模式()&&modules.length>1?'<button type="button" class="module-delete" data-map-delete-module="'+逃(sourceId)+'">删除</button>':'')+'</div>'}).join('');
    const cloneButton=可编辑模式()&&!selected.inStaging?'<button type="button" class="clone-mini" data-map-clone="'+逃(selected.id)+'">新增模块</button>':'';
    const moduleHtml='<div class="module-management"><div class="module-title"><b>当前SKU陈列模块 '+displayModules.length+' 个</b><small>当前为模块'+moduleIndex+'</small></div>'+moduleRows+(cloneButton?'<div class="module-clone-options"><span>新增后进入待选区，请手动迁移到目标柜段</span>'+cloneButton+'</div>':'')+'</div>';
  ...
  const stagedItemsHtml=staged.map(r=>'<span class="map-item staging-item" data-sku-id="'+逃(r.id)+'" draggable="true" style="'+陈列图商品样式(r)+'">'+陈列图商品标签(r)+'</span>').join('');
...
function 渲染陈列图(){
  ...
  let html='<div class="map-store-title">'+逃(store)+(type4?' · '+逃(type4):'')+'</div>'+(type4?'<div class="help planogram-filter-note">当前只显示“'+逃(type4)+'”品类；柜段已用和余量仍按该柜段全部陈列计算，切换为“全部四级品类”可查看完整陈列。</div>':'');
...
a.download=name;
a.click();
URL.revokeObjectURL(a.href)}q("#storeSelect").onchange=e=>{提交当前编辑();当前.门店=e.target.value;当前.陈列图四级="";当前.陈列图选中SKU="";
渲染全部()};
...
function 新店柜段预览项目(c,rows){
  const keys=new Set((c.items||[]).map(x=>typeof x==="string"?x:(x?.skuKey||x?.id)).filter(Boolean));
  return rows.filter(r=>r.included&&(r.cabinetKey===c.key||keys.has(r.skuKey)||keys.has(r.id)));
}
function 新店剔除分(r){
...
渲染新增门店=function(){
  ...
  const previewCabs=pre.cabinets.map(c=>({...c,previewItems:新店柜段预览项目(c,pre.skus)}));
  表格('#newStoreCabinetPreview',[{name:'冰柜类型',value:c=>c.kind},{name:'陈列柜',value:c=>c.label,cls:()=> 'name'},{name:'具体位置',value:c=>c.position},{name:'长',value:c=>格(c.length,0)},{name:'已用',value:c=>格(c.sourceUsed,0)},{name:'剩余',value:c=>格(c.sourceLeft,0),cls:c=>c.sourceLeft<0?'bad':c.sourceLeft>300?'warn':'ok'},{name:'SKU数',value:c=>c.previewItems?.length||0},{name:'占用品',value:c=>(c.previewItems||[]).map(x=>x.name+' '+格(x.displayCols,0)+'列').join('；'),cls:()=> 'name'}],previewCabs);
