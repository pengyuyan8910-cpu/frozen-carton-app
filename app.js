...
const opsViews=new Set(["allocation","cabinets","suggestions","newstore","io"]);
if(!isOps&&opsViews.has(当前.页面)){当前.页面="store"}
const banner=q("#modeBanner");
if(banner)banner.textContent=isOps?"当前为运营模式：可在陈列图调整商品位置、列数、满陈与外储；点击“同步至店员端”后，门店按当前方案执行。":"";
...
function 新店重算用量(pre){
  ...
  if(pre.strictEngine&&pre.enginePlan&&globalThis.StrictAllocationAdapter?.recalculatePlan){
    ...
    globalThis.StrictAllocationAdapter.recalculatePlan(pre.enginePlan,{productPool:产品池有效(),externalCapL:状态.params.externalCapL});
...
预排新增门店=function(store,type,cabs){
  ...
  if(adapter?.allocateStore){
    const plan=adapter.allocateStore({store,type,productPool:产品池有效(),cabinets:cabs,params:状态.params,physicalRecords:[]},{maxIterations:12,maxExpansions:180});
