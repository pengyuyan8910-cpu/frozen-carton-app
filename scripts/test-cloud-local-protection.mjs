...
const start = source.indexOf('function cloudProtectCurrentPage()');
...
assert.equal(protect(), true, '本机主状态保存成功时，云端保护不应因重复回退副本失败');
...
console.warn = () => {};
try {
  assert.equal(protect(), true, '回退标记写入失败时，已成功保存的本机主状态仍应保护当前页面');
} finally {
