export const DETERMINISTIC_SEARCH_BUDGET = Object.freeze({
  ...
  phase5OptimizationBatchRounds: 20,
  phase5MaxOptimizationBatches: 10,
...
export function resolveDeterministicSearchBudget(options = {}) {
  const optimizationBatchRounds = Math.min(50, Math.max(1, Math.floor(Number(
    options.optimizationBatchRounds
      ?? options.maxRounds
      ?? DETERMINISTIC_SEARCH_BUDGET.phase5OptimizationBatchRounds
  ))));
  return Object.freeze({
    ...
    optimizationBatchRounds,
    maxRounds: optimizationBatchRounds,
    maxOptimizationBatches: Math.min(50, Math.max(1, Math.floor(Number(
      options.maxOptimizationBatches ?? DETERMINISTIC_SEARCH_BUDGET.phase5MaxOptimizationBatches
    )))),
