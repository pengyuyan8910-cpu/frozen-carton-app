...
async function main() {
  ...
  if (migrateAccepted) {
    ...
    if (!data) throw new Error("67SKU确认版基准读取失败");
  } else {
  ...
  if (migrateAccepted) {
    const tempWorkbook = path.join(sourceDir, ".67SKU确认版迁移.xlsx");
  ...
  if (process.env.GITHUB_STEP_SUMMARY && migrateAccepted) fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY, "## 67SKU确认版已设为正式基准\n\n宁国、和县、无为等门店将以用户确认方案作为后续幂等重排基准。\n", "utf8");
