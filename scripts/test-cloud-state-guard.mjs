...
assert.equal(shouldInitializeCloud(null), true, '没有基线时必须以当前页面初始化云端');
assert.equal(shouldInitializeCloud({ initialized: false }), true);
assert.equal(evaluateCloudPull({ baseline: null, remote: { doc_revision: 9, payload: { old: true } } }).action, 'initialize-current');
...
assert.equal(evaluateCloudPull({ baseline: { ...baseline, initialized: false }, remote: { doc_revision: 1, payload: { old: true } } }).action, 'initialize-current');
